...
**Values that render inside a line get the other framing.** `displayName` in a `###` author heading, a trend's topic name, a moderation label's `val` — a blockquote is the wrong shape for these, since quoting them would push the heading they belong to onto a line of its own. `inlineUserText()` folds their line breaks to spaces instead, which is the only escape those positions are open to, and strips nothing else. This is not hypothetical: `app.bsky.actor.profile` bounds `displayName` by graphemes alone (64) and excludes no character, and 1 of 3,929 live display names sampled already carried a line break. `label.val` is bounded at 128 characters with no pattern and is written by third-party labelers, which live data confirms is no controlled vocabulary — `reaction_count:19` and `Adult Content` both appear. `handle` is the one identity field left alone: the lexicon gives it `format: "handle"`, so it is a dotted domain by construction.
...
- **Parent chains longer than `parent_height` end without a marker**: The AppView stops the chain at the requested height and gives no signal that it did. A caller reading a chain longer than 100 posts must notice the topmost returned parent still carries `replyToUri` and re-root there.
